...
{
    let playerData = await fetch("https://earthmc.net/map/aurora/up/world/earth/").then(response => response.json()).catch(err => {})  

    if (!playerData || !playerData.players) return
    else return playerData
}

async function getFastPlayerData()
{
    let playerData = await fetch("https://fastmap.earthmc.net/tiles/players.json").then(response => response.json()).catch(err => {})  
...
async function getOnlinePlayerData2()
{
    let playerData = await getPlayerData()
	var pData = fn.editPlayerProps(playerData.players)
	//var player = pData.split(',');
	//var data = Object.values(pData);
	console.log(pData.name)
	//console.log(data)

    if (!playerData || !playerData.players) return
    else return fn.editPlayerProps(data.players)
}

async function getMapData()
{
    let mapData = await fetch("https://earthmc.net/map/aurora/tiles/_markers_/marker_earth.json").then(response => response.json()).catch(err => {})   
...
async function getTowns2()
{
    ...
    {      
        ...
        var townName = info[0].split(" (")[0].trim()          
        if (townAreaNames[i].endsWith("Shop")) continue
...
async function getOnlinePlayers(includeResidentInfo)
{
		...
		for (let i = 0; i < onlinePlayers.length; i++) 
    {
		...
		players.push(onlinePlayers[i].name)
	}
	return players
	}
...
async function getInvitableTowns(nationName, includeBelonging)
{
    ...
    {
        if (includeBelonging) return Math.hypot(town.x - nation.capitalX, town.z - nation.capitalZ) <= 3500 && town.nation != nationName
        else return Math.hypot(town.x - nation.capitalX, town.z - nation.capitalZ) <= 3500 && town.nation != nationName && town.nation == "No Nation"
    }
...
async function getJoinableNations(townName)
{
    ...
    {
        return Math.hypot(nation.capitalX - town.x, nation.capitalZ - town.z) <= 3500
    }
